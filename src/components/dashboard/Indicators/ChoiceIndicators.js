...
import styles from "./ChoiceIndicators.module.css";
...
export default function ChoiceIndicators({ onNext }) {
  ...
  useEffect(() => {
    ...
    async function loadAxes() {
      try {
        const response = await fetchWithAuth("/api/indicadores/user-indicator-selection/");
    ...
    <section className={styles.containerStrategicPlatform}>
      <div className={styles.StrategicPlatform}>
        <h2 className={styles.titule}>
          <span className="spanDoarado">Indicadores </span>
        </h2>
      ...
      <div className={styles.containerAdvertising}>
        <img src="/img/dashboard/bg/indicadores.webp" alt="Publicidad" className={styles.adImage} />
      </div>
    </section>
  );
}
