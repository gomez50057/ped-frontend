  ...
  return (
    <div className={styles.container}>
      {allIndicators.map((item) => (
        <div key={item.indicador.id} className={styles.indicatorCard}>
          {/* Header */}
          <div className={styles.header}>
            <span className={styles.headerText}>FICHA DEL INDICADOR {item.indicador.id.replace('indicador_', '').replace(/_/g, '. ').toUpperCase()}</span>
          </div>
                ...
                <div className={styles.odsContainer}>
                  {item.alineacion_ODS.ods.map((ods) => (
                    <div key={ods} className={styles.odsItem}>
                      {/* Si tienes imágenes de ODS, reemplaza src por el link correspondiente */}
                      <img
